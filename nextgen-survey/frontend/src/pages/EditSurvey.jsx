...
const EditSurvey = () => {
  ...
  useEffect(() => {
    if (!survey) {
      toast.error('Survey parameters not found. Redirecting...');
      navigate('/dashboard');
      return;
    }
    ...
    setIsPublic(survey.isPublic !== undefined ? survey.isPublic : true);
  }, [survey, navigate]);
  ...
  const updateQuestion = (id, updates) => {
    setQuestions(questions.map(q => (q.id === id || q._id === id) ? { ...q, ...updates } : q));
  };

  const handleSave = async () => {
    if (!title || questions.length === 0) return toast.error('Survey needs a title and at least one question');
    const savingToast = toast.loading('Syncing modifications to primary database...');
    setSaving(true);
    try {
      await api.updateSurvey(survey._id, { title, description, questions, isPublic });
      toast.success('Survey updated flawlessly!', { id: savingToast });
      navigate('/dashboard');
    } catch (err) {
      toast.error(err.message || 'Failed to update survey', { id: savingToast });
    } finally {
      setSaving(false);
    }
  };
    ...
    <div className="max-w-4xl mx-auto space-y-8 pb-32">
      ...
      <div className="bg-gradient-to-r from-blue-600 to-indigo-600 rounded-2xl p-6 text-white shadow-lg mb-8 flex justify-between items-center">
         <div>
            ...
            <p className="text-blue-100 mt-1">Changes are pushed instantly to all respondents upon saving.</p>
         </div>
      </div>
          ...
          {questions.map((q) => (
            ...
            >
              <div className="flex justify-between items-start mb-6">
                ...
                <button 
                  onClick={() => removeQuestion(q.id || q._id)}
              ...
              <input 
                ...
                onChange={(e) => updateQuestion(q.id || q._id, { questionText: e.target.value })}
              ...
              {(q.type === 'MCQ' || q.type === 'Dropdown' || q.type === 'Checkbox') && (
                <div className="space-y-3 ml-2 border-l-2 border-gray-200 dark:border-gray-700 pl-4">
                    ...
                    <div key={i} className="flex items-center gap-3">
                      ...
                      <input 
                        ...
                        onChange={(e) => {
                          ...
                          updateQuestion(q.id || q._id, { options: newOpts });
                        }}
                      />
                      <button onClick={() => {
                  ...
                  <button 
                    onClick={() => updateQuestion(q.id || q._id, { options: [...q.options, `Option ${q.options.length + 1}`] })}
      ...
      <div className="fixed bottom-0 left-0 w-full bg-white/90 dark:bg-dark-800/90 backdrop-blur-md border-t border-gray-200 dark:border-gray-800 p-4 flex justify-end px-10 z-20 shadow-[0_-10px_20px_rgba(0,0,0,0.05)]">
        <button 
          onClick={handleSave} 
          disabled={saving}
          className="flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white font-bold px-8 py-3 rounded-xl shadow-lg shadow-indigo-500/30 transition-all disabled:opacity-50"
        >
          <Save className="w-5 h-5" /> {saving ? 'Syncing...' : 'Update Active Survey'}
        </button>
      </div>
    </div>
  );
